...
- [x] Phase 1: 100 以内加减法速算（二年级）— 确定性出题 + 判分 + 计时
- [x] Phase 2: 表内乘除法（二~三年级，九九表，除法保证整除）
- [x] Phase 3: 多位数加减法（三年级，千以内，最小操作数 10）
- [x] Phase 4: 小数初步认识与计算（四年级，一位/两位小数加减）
- [ ] Phase 5: 简易方程与几何（五年级）
- [ ] Phase 6: 语文 / 英语辅助模块（待规划）
- [x] Phase 6.5: 多人竞技（server relay 对战房间，竞赛码即房间号，2s 轮询排行榜）
- [x] Phase 7a: 画像反哺出题雏形——进退位正确率 <80% 自动加强进退位题
- [x] Phase 7b-1: 错因聚类（看错符号/进退位失误/计算错误，看板可视化 + 建议）
- [ ] Phase 7b-2: 知识点级动态推荐
