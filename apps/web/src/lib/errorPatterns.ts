...
export function dominantAdvice(stat: PatternStat): string {
  ...
  const tips: Record<ErrorKind, string> = {
    ...
    word: `最近有 ${n} 次字词选错，把错词多读几遍、抄一遍，印象会更牢`,
  }
  return tips[kind]
}
